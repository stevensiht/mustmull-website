...
const successMessage = 'Oleme Teie päringu kätte saanud. Vastame teile 48 tunni jooksul.';
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data?.error ?? 'Saatmine ebaõnnestus. Palun proovi uuesti.');
      }
      ...
      setStatus('error');
      setFeedback(
        error instanceof Error ? error.message : 'Saatmine ebaõnnestus. Palun proovi uuesti.'
      );
    }
  };
    ...
    >
      ...
      <div className='mx-auto flex max-w-4xl flex-col gap-8 px-6 text-center sm:px-8'>
        ...
        <p className='text-base text-white/70 sm:text-lg'>
          Tutvusta meile oma planeeritavat üritust, lisa kokteilieelistused ja anna teada külaliste
          arvust - nii saame sulle parima pakkumise koostada.
        </p>
        <form className='grid gap-6 text-left' onSubmit={handleSubmit} noValidate>
          {status === 'success' && feedback && (
            ...
            >
              ...
              <p className='text-sm font-medium text-white sm:text-base'>{feedback}</p>
            </div>
          )}
          ...
          <div className='grid gap-2 sm:grid-cols-2 sm:gap-4 min-w-0'>
          ...
          <div className='grid gap-2 sm:grid-cols-2 sm:gap-4'>
            <div className='grid gap-2'>
              ...
              <input
                ...
                required
                disabled={isSubmitting}
                className='w-full max-w-full min-w-0 rounded-2xl border border-white/15 bg-background/80 px-4 py-3 text-sm text-white placeholder:text-white/40 focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/40 disabled:cursor-not-allowed disabled:opacity-60 appearance-none [-webkit-appearance:none]'
              />
            </div>
            <div className='grid gap-2'>
              <label
          ...
          <div className='flex flex-col items-center gap-3 text-center sm:flex-row sm:justify-between'>
            ...
            >
              {status === 'success' ? defaultStatusMessage : (feedback ?? defaultStatusMessage)}
            </p>
            <button
              type='submit'
              disabled={isSubmitting}
